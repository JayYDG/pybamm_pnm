...
def run_simulation_lp_double(parameter_values, experiment, initial_soc, project, dtheta, Rs=1e-3, Ri=90, thermal=True):
    """
    Will simulate a jelly with double layer between inner and outerlayer.
    (Outer layer and inner layer is single sided, the layer between them are double signded)

    Parameter:
    thermal: If True, it will consider thermal effect. If false, it is isothermal.
    """
    ###########################################################################
    ...
    # I_app = 0.5
    netlist = ecm.network_to_netlist_double(dtheta, net, Rs, Ri, V, I_app)
    netlist = ecm.network_update_double_R(netlist)
    T0 = parameter_values["Initial temperature [K]"]
